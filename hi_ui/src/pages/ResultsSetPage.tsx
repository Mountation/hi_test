...
const ResultsSetPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      setResults(await api.listResultsBySet(setId));
    } catch (e:any) {
      console.error('Failed to load results by set', e);
  ...
  const columns = [
    ...
    { title: 'DataID', dataIndex: 'eval_data_id', key: 'eval_data_id', width: 100 },
